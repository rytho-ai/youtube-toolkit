...
|---|---|
| handler fallback 順序 + logging 行為 | `core/fallback.py`（`run_with_fallback`）。已採用於 canonical「依序試→全失敗 raise」站點（get_video_info、download_captions），並為新代碼的標準（見 CLAUDE.md/EXTENDING.md）。result-guard / 回傳預設 / runtime 決定順序等不同形狀的 fallback-like 方法刻意保留手刻，不過度抽象（rule ④） |
